  ...
  std::vector<cv::Mat> & tvecs)
{
  ...
  for (int i = 1; true; i++) {
    ...
    oss << R_gimbal2world;
    tools::logger()->info("R_gimbal2world:\n{}", oss.str());
    ...
    auto success = cv::findChessboardCorners(gray, pattern_size, centers_2d);
...
int main(int argc, char * argv[])
{
  ...
  // 数据验证
  if (rvecs.size() < 3) {
    fmt::print(stderr, "错误: 数据量不足，至少需要3组数据，当前只有{}组\n", rvecs.size());
    return 1;
  }
  ...
  for (size_t i = 0; i < R_world2gimbal_list.size(); i++) {
    cv::Mat R = R_world2gimbal_list[i];
    double det = cv::determinant(R);
    if (std::abs(det - 1.0) > 0.1) {
      fmt::print(stderr, "警告: 第{}组数据的旋转矩阵行列式为{:.6f}，不是有效的旋转矩阵\n", i + 1, det);
    }
    // 检查是否为正交矩阵
    cv::Mat R_transpose, should_be_identity;
    cv::transpose(R, R_transpose);
    should_be_identity = R * R_transpose;
    cv::Mat identity = cv::Mat::eye(3, 3, CV_64F);
    cv::Mat diff = should_be_identity - identity;
    double norm = cv::norm(diff);
    if (norm > 0.01) {
      fmt::print(stderr, "警告: 第{}组数据的旋转矩阵不是正交矩阵，误差: {:.6f}\n", i + 1, norm);
    }
  }
  ...
  for (size_t i = 0; i < R_world2gimbal_list.size(); i++) {
    for (size_t j = i + 1; j < R_world2gimbal_list.size(); j++) {
      Eigen::Matrix3d R_i, R_j;
      cv::cv2eigen(R_world2gimbal_list[i], R_i);
      cv::cv2eigen(R_world2gimbal_list[j], R_j);
      Eigen::Matrix3d R_diff = R_i.transpose() * R_j;
      Eigen::AngleAxisd angle_axis(R_diff);
      double angle = std::abs(angle_axis.angle());
      max_rotation_diff = std::max(max_rotation_diff, angle);
    }
  }
  fmt::print("最大旋转角度差: {:.2f} 度\n", max_rotation_diff * 57.3);
  if (max_rotation_diff < 0.1) {
    fmt::print(stderr, "错误: 所有数据之间的旋转变化太小（< 5.7度），无法进行标定\n");
    fmt::print(stderr, "请确保采集数据时云台有足够大的角度变化\n");
    return 1;
  }
  ...
  try {
    ...
    fmt::print(stderr, "可能的原因:\n");
    fmt::print(stderr, "1. 数据量不足或质量差\n");
    fmt::print(stderr, "2. 旋转变化不够大\n");
    fmt::print(stderr, "3. R_gimbal2imubody 配置错误\n");
    fmt::print(stderr, "4. 标定板识别不准确\n");
    return 1;
  }
  ...
  Eigen::Vector3d camera_ypr = tools::eulers(R_camera2ideal, 2, 1, 0) * 57.3;  // degree
